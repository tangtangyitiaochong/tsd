...
<html>
	...
	<body>
				...
				<div class="navbar-header pull-right" role="navigation">
					<ul class="ace-nav">
						<li>
							...
							<span class="white">欢迎光临，${session.userInfo.userName}></span>
		...
		$(document).ready(function(){
			...
			$.ajax({     
    					url:'${pageContext.request.contextPath }/menuTree.action',
						data:{
							"roleId":roleId,
						},
    					type:'post',     
    					...
    					success:function(data){
							//var tBsMenuInfos = data.tBsMenuInfos;
							...
							$(data).each(function(index) {
                				...
                				if(menu.parentId==0){
									...
									count = 1;
									//构建根节点的子节点
                					buildMenu(menu.menuId,menu.menuName,data);
                					html += "</ul>";
                					html += "</li>";
                				}
            				});
		...
		//构造树，出去根节点
		function buildMenu(menuId,menuName,data){
				$(data).each(function(index) {
            	var menu = data[index];
            	if(menuId==menu.parentId){
            		if(hasChild(menu.menuId,data)>0){//如果有子节点 构造<ul>
            			count++;
            			html += 
            				"<li>" +
